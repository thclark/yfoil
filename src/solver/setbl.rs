...
pub fn build_newton_system(
    ...
    // Total system size: upper + lower surfaces (wake handled separately)
    let nsys = state.nbl_upper + state.nbl_lower;
    ...
    let mut input = BlsolvInput {
        ...
        vz: [[0.0; 2]; 3],
        ivte1: Some(state.nbl_upper - 1), // Upper TE system index
        ivz: Some(state.nbl_upper),       // Lower surface start
    ...
    if let Some(dij) = inviscid.get_dij() {
        add_dij_coupling(state, &mut input, dij, airfoil.le_index);
    }
...
///
/// Marches the BL stations and fills the VA, VB, VDEL, VM matrices.
fn build_surface_system(
    ...
    // Initialize first station (similarity solution)
    let s2_init = init_similarity_station(xssi[0], uedg[0], &state.params);
    stations[0] = s2_init.clone();

    // Local system for building Jacobian
    let mut local_sys = BLLocalSystem::default();

    // March through stations
    for ibl in 1..nbl {
        let iv = offset + ibl; // Global system index

        // Clone previous station state to avoid borrow conflict
        let s1 = stations[ibl - 1].clone();

        // Initialize s2 from current data or extrapolation
        let mut s2_init = BLStationState::default();
        s2_init.blprv(
            xssi[ibl],
            s1.ampl,      // Amplification from previous
            s1.ctau,      // Ctau from previous
            s1.theta * 1.1, // Slight growth
            s1.dstar * 1.1,
            0.0,          // No wake gap on surface
            uedg[ibl],
            &state.params,
        );
        ...
        // March this station (single iteration for system building)
        ...
        // Build local system before storing s2
        let is_simi = ibl == 1; // Similarity station is IBL=2 in XFOIL terms
        ...
        // Copy local system to global matrices
        copy_to_global_system(&local_sys, &s2, input, iv, is_simi);

        // Store s2 in stations array
        stations[ibl] = s2;
    }
}

/// Initialize similarity (stagnation) station
///
/// This is equivalent to XFOIL's Thwaites solution at the leading edge.
fn init_similarity_station(
    ...
    let theta = (0.45 * nu * xsi.max(1e-10) / (6.0 * uei.max(0.01))).sqrt();
    let h = 2.2; // Hiemenz stagnation point
    let dstar = h * theta;

    let mut s = BLStationState::default();
    s.blprv(xsi, 0.0, 0.03, theta, dstar, 0.0, uei, params);
    s.blkin(params);
...
fn add_dij_coupling(
    ...
    for iv in 0..nsys {
        // Get panel index for station iv
        let ipan_i = if iv < state.nbl_upper {
            state.ipan_upper[iv]
        } else {
            state.ipan_lower[iv - state.nbl_upper]
        };

        // Get station state for derivatives
        let s_i = if iv < state.nbl_upper {
            &state.stations_upper[iv]
        } else {
            &state.stations_lower[iv - state.nbl_upper]
        };

        // VTI sign for station i
        let vti_i = if ipan_i <= le_index { 1.0 } else { -1.0 };

        for jv in 0..nsys {
            let ipan_j = if jv < state.nbl_upper {
                state.ipan_upper[jv]
            } else {
                state.ipan_lower[jv - state.nbl_upper]
            };

            // VTI sign for station j
            let vti_j = if ipan_j <= le_index { 1.0 } else { -1.0 };
...
pub fn apply_newton_update(
    ...
    // Apply updates to upper surface
    for ibl in 0..state.nbl_upper {
        let iv = ibl;
        let delta = &vdel[iv];
        let station = &mut state.stations_upper[ibl];

        let (_rlx, dm) = apply_station_update(station, delta, config);
        rmsbl += dm * dm;
        dmax = dmax.max(dm);
        n_changed += 1;
    ...
    // Apply updates to lower surface
    for ibl in 0..state.nbl_lower {
        let iv = state.nbl_upper + ibl;
        let delta = &vdel[iv];
    ...
    // Compute RMS
    rmsbl = if n_changed > 0 {
        (rmsbl / n_changed as f64).sqrt()
    } else {
        0.0
    };
...
///
/// Returns (relaxation_factor, max_normalized_change)
fn apply_station_update(
    ...
    // Limit increases
    if rel_ctau > config.dhi {
        rlx = rlx.min(config.dhi / rel_ctau);
    }
    if rel_theta > config.dhi {
        rlx = rlx.min(config.dhi / rel_theta);
    }
    if rel_mass > config.dhi {
        rlx = rlx.min(config.dhi / rel_mass);
    }
    ...
    // Maximum normalized change for RMSBL
    let dmax = rel_ctau.abs().max(rel_theta.abs()).max(rel_mass.abs()) * rlx;
...
/// * `le_index` - Leading edge index for VTI computation
///
/// # Returns
...
pub fn update_edge_velocities(
    ...
    for (ibl, &ipan) in state.ipan_upper.iter().enumerate() {
        if ibl < state.nbl_upper {
            mass[ipan] = state.upper.mass[ibl];
        }
    }
    ...
    for (ibl, &ipan) in state.ipan_lower.iter().enumerate() {
        if ibl < state.nbl_lower {
            mass[ipan] = state.lower.mass[ibl];
        }
    }

    // Apply DIJ coupling
    for i in 0..n {
        let vti_i = if i <= le_index { 1.0 } else { -1.0 };
        for j in 0..n {
            let vti_j = if j <= le_index { 1.0 } else { -1.0 };
            ue[i] += -vti_i * vti_j * dij[(i, j)] * mass[j];
        }
        // Ensure positive
        ue[i] = ue[i].max(0.01);
    }

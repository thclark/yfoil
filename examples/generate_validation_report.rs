...
//!
//! Run with: cargo run --example generate_validation_report
...
fn write_summary_report(results: &[ValidationResult], output_dir: &Path) -> std::io::Result<()> {
    ...
    writeln!(file, "# YFoil Subroutine Validation Report")?;
    writeln!(file)?;
    writeln!(file, "This report compares YFoil's closure functions against XFOIL reference values.")?;
    writeln!(file)?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output_dir = project_root.join("docs/xfoil-reference/validation");
